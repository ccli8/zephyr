 ...
 * 2. Some soc series don't allow ISO IN/OUT to be assigned the same EP number.
 *    There is one solution but it can only work for not more than one ISO IN
 *    EP enabled, or behavior is undefined.
 *    See CONFIG_USB_DC_NUMAKER_ALLOW_ISO_IN_OUT_SAME_NUM.
 */
...
#define NUMAKER_USBD_EP_MAXNUM USBD_MAX_EP
...
#if defined(CONFIG_USB_DC_NUMAKER_ALLOW_ISO_IN_OUT_SAME_NUM)
#define NUMAKER_USBD_ALLOW_ISO_IN_OUT_SAME_NUM_REG 0x001C
#define NUMAKER_USBD_ALLOW_ISO_IN_OUT_SAME_NUM_Msk 0x2
#endif
...
struct numaker_usbd_ep {
	...
	uint8_t usbd_hw_ep_hndl; /* BSP USBD driver EP handle EP0, EP1, EP2, etc */
	uint32_t usbd_hw_ep_cfg; /* BSP USBD driver EP configuration */
...
static void numaker_usbd_remote_wakeup(const struct device *dev)
{
	...
	/* Then generate 'K' */
	config->base->ATTR |= USBD_ATTR_RWAKEUP_Msk;
...
static void numaker_usbd_setup_fifo_copy_to_user(const struct device *dev, uint8_t *usrbuf)
{
	...
	uint32_t dmabuf_addr =
		USBD_BUF_BASE + (config->base->STBUFSEG & USBD_STBUFSEG_STBUFSEG_Msk);
					     ...
					     uint32_t *size_p)
{
	...
	dmabuf_addr = USBD_BUF_BASE + ep_cur->read_fifo_pos;
					       ...
					       const uint8_t *usrbuf, uint32_t *size_p)
{
	...
	dmabuf_addr = USBD_BUF_BASE + USBD_GET_EP_BUF_ADDR(ep_cur->usbd_hw_ep_hndl);

	*size_p =
		MIN(*size_p, numaker_usbd_ep_fifo_max(ep_cur) - numaker_usbd_ep_fifo_used(ep_cur));
...
static void numaker_usbd_ep_abort(struct numaker_usbd_ep *ep_cur)
{
	...
	if (ep_cur->usbd_hw_ep_cfg & USBD_CFG_TYPE_ISO) {
		...
		uintptr_t reg =
			((uintptr_t)config->base) + NUMAKER_USBD_ALLOW_ISO_IN_OUT_SAME_NUM_REG;
		*((volatile uint32_t *)reg) |= NUMAKER_USBD_ALLOW_ISO_IN_OUT_SAME_NUM_Msk;
	}
#endif
					 ...
					 const struct usb_dc_ep_cfg_data *const ep_cfg)
{
	...
	if (ep_cfg->ep_type == USB_DC_EP_CONTROL) {
		ep_cur->usbd_hw_ep_cfg |= USBD_CFG_CSTALL;
	}

	/* Default to DATA0 */
	ep_cur->usbd_hw_ep_cfg &= ~USBD_CFG_DSQSYNC_Msk;

	/* Endpoint IN/OUT, though, default to disabled */
	ep_cur->usbd_hw_ep_cfg |= USBD_CFG_EPMODE_DISABLE;

	/* Isochronous or not */
	if (ep_cfg->ep_type == USB_DC_EP_ISOCHRONOUS) {
		ep_cur->usbd_hw_ep_cfg |= USBD_CFG_TYPE_ISO;
	}

	/* Endpoint index */
	ep_cur->usbd_hw_ep_cfg |=
...
static void numaker_usbd_ep_enable(struct numaker_usbd_ep *ep_cur)
{
	...
	/* Enable EP to IN/OUT */
	ep_cur->usbd_hw_ep_cfg &= ~USBD_CFG_STATE_Msk;
	if (USB_EP_DIR_IS_IN(ep_cur->addr)) {
		ep_cur->usbd_hw_ep_cfg |= USBD_CFG_EPMODE_IN;
	} else {
		ep_cur->usbd_hw_ep_cfg |= USBD_CFG_EPMODE_OUT;
	}
...
static void numaker_usbd_ep_disable(struct numaker_usbd_ep *ep_cur)
{
	...
	/* Disable EP */
	ep_cur->usbd_hw_ep_cfg =
		(ep_cur->usbd_hw_ep_cfg & ~USBD_CFG_STATE_Msk) | USBD_CFG_EPMODE_DISABLE;
	USBD_CONFIG_EP(ep_cur->usbd_hw_ep_hndl, ep_cur->usbd_hw_ep_cfg);
}
...
static void numaker_usbd_ep_mgmt_init(const struct device *dev)
{
	...
	for (; ep_cur != ep_end; ep_cur++) {
		...
		ep_cur->usbd_hw_ep_hndl = EP0 + (ep_cur - ep_mgmt->ep_pool);
	}
	...
	config->base->STBUFSEG = ep_mgmt->dmabuf_pos;
...
static void numaker_udbd_isr(const struct device *dev)
{
	...
	uint32_t volatile usbd_intsts = USBD_GET_INT_FLAG();
	uint32_t volatile usbd_bus_state = USBD_GET_BUS_STATE();
	...
	if (usbd_intsts & USBD_INTSTS_USB) {
		...
		while (epintsts) {
			...
			uint8_t ep_dir = ((config->base->EP[hw_ep_idx].CFG & USBD_CFG_STATE_Msk) ==
					  USBD_CFG_EPMODE_IN)
						 ? USB_EP_DIR_IN
						 : USB_EP_DIR_OUT;
			uint8_t ep_idx = (config->base->EP[hw_ep_idx].CFG & USBD_CFG_EPNUM_Msk) >>
					 USBD_CFG_EPNUM_Pos;
			uint8_t ep = USB_EP_GET_ADDR(ep_idx, ep_dir);
			...
			if (ep == USB_EP_GET_ADDR(0, USB_EP_DIR_OUT)) {
				...
				ep_ctrlout->mxpld_ctrlout =
					USBD_GET_PAYLOAD_LEN(ep_ctrlout->usbd_hw_ep_hndl);
			}

#if defined(CONFIG_USB_DC_NUMAKER_ALLOW_ISO_IN_OUT_SAME_NUM)
			if (config->base->EP[hw_ep_idx].CFG & USBD_CFG_TYPE_ISO) {
				/* Restore to not enabling the function */
				uintptr_t reg = ((uintptr_t)config->base) +
						NUMAKER_USBD_ALLOW_ISO_IN_OUT_SAME_NUM_REG;
				*((volatile uint32_t *)reg) |=
					NUMAKER_USBD_ALLOW_ISO_IN_OUT_SAME_NUM_Msk;
			}
#endif
...
int usb_dc_ep_configure(const struct usb_dc_ep_cfg_data *const ep_cfg)
{
	...
	LOG_DBG("Configure: ep=0x%02x, ep_mps=%d, ep_type=%d", ep_cfg->ep_addr, ep_cfg->ep_mps,
		ep_cfg->ep_type);
...
#define USB_DC_NUMAKER_INIT(inst)                                                                  \
	...
	static struct numaker_usbd_data numaker_usbd_data_##inst;                                  \
                                                                                                   \
	BUILD_ASSERT(DT_INST_PROP(inst, num_bidir_endpoints) == USBD_MAX_EP,                       \
