...
/* UTCPD VBUS threshold default in mV
 *
 ...
 */
#define NUMAKER_UTCPD_VBUS_THRESHOLD_OVERVOLTAGE          25000
#define NUMAKER_UTCPD_VBUS_THRESHOLD_VSAFE5V              5000
#define NUMAKER_UTCPD_VBUS_THRESHOLD_VSAFE0V              0
#define NUMAKER_UTCPD_VBUS_THRESHOLD_STOP_FORCE_DISCHARGE 800
#define NUMAKER_UTCPD_VBUS_THRESHOLD_SINK_DISCONNECT      3500

/* This goto replacement is to silence checkpatch complaint:
 * WARNING:MACRO_WITH_FLOW_CONTROL: Macros with flow control statements should be avoided
 *
 * Use of goto for single exit point (cleanup) is decent. Here, its use
 * can make code e.g. UTCPD register write with error condition more concise.
 */
#define NUMAKER_GOTO UTIL_CAT(go, to)

/* Helper which makes the boilerplate code for 'rc' expression check */
#define NUMAKER_RC_CHECK(rc_expr)                                                                  \
	rc = (rc_expr);                                                                            \
	if (rc < 0) {                                                                              \
		NUMAKER_GOTO cleanup;                                                              \
	}
#define NUMAKER_RC_CHECK_WITH_MSG(rc_expr, msg_on_err)                                             \
	rc = (rc_expr);                                                                            \
	if (rc < 0) {                                                                              \
		LOG_ERR(msg_on_err);                                                               \
		NUMAKER_GOTO cleanup;                                                              \
...
/* UTCPD register write timeout in microseconds */
#define NUMAKER_UTCPD_REG_WRITE_BY_NAME_TIMEOUT_US 20000

/* UTCPD register read/write */

#define NUMAKER_UTCPD_REG_WRITE_BY_NAME(dev, reg_name, val)                                        \
	{                                                                                          \
		rc = numaker_utcpd_reg_write_wait_ready(dev);                                      \
		if (rc < 0) {                                                                      \
...
#define NUMAKER_UTCPD_REG_FORCE_WRITE_BY_NAME(dev, reg_name, val)                                  \
	{                                                                                          \
		...
		if (rc < 0) {                                                                      \
			LOG_ERR("UTCPD register (%s) write timeout", #reg_name);                   \
			LOG_ERR("UTCPD register (%s) force-write", #reg_name);                     \
		}                                                                                  \
		utcpd_base->reg_name = (val);                                                      \
	}
...
#define NUMAKER_UTCPD_REG_FORCE_WRITE_BY_OFFSET(dev, reg_offset, val)                              \
	{                                                                                          \
		...
		if (rc < 0) {                                                                      \
			LOG_ERR("UTCPD register (0x%04x) write timeout", reg_offset);              \
			LOG_ERR("UTCPD register (0x%04x) force-write", reg_offset);                \
		}                                                                                  \
		sys_write32((val), ((uintptr_t)utcpd_base) + reg_offset);                          \
	}
...
/* Helper to read UTCPD VBUS threshold */
...
/* Wait ready for next write access to UTCPD register */
...
/* Convert VBUS voltage format between H/W bit and mV
 *
...
static uint32_t numaker_utcpd_vbus_volt_bit2mv(const struct device *dev, uint32_t bit)
{
	...
	return bit * data->vref_mv * config->utcpd.vbvol.vbscale.value / BIT_MASK(10);
}
...
static int numaker_eadc_vref_init(const struct device *dev)
{
	...
	if (spec) {
		/* ADC device ready */
		NUMAKER_RC_CHECK_WITH_MSG((adc_is_ready_dt(spec) ? 0 : -ENODEV),
					  "ADC device for VBUS/VCONN not ready");
		/* ADC channel configuration ready */
		NUMAKER_RC_CHECK_WITH_MSG((spec->channel_cfg_dt_node_exists ? 0 : -ENODEV),
					  "ADC channel configuration for VBUS/VCONN not specified");
		...
		if (reference == ADC_REF_EXTERNAL0 || reference == ADC_REF_EXTERNAL1) {
			...
			case 3000:
				...
				break;
			default:
				NUMAKER_RC_CHECK_WITH_MSG(-ENOTSUP, "Invalid Vref voltage");
			}
		} else {
			NUMAKER_RC_CHECK_WITH_MSG(-ENOTSUP, "Invalid Vref source");
		}
...
int numaker_utcpd_vbus_measure(const struct device *dev, uint32_t *mv)
{
	...
	NUMAKER_RC_CHECK(mv != NULL ? 0 : -EINVAL);
	*mv = 0;

	NUMAKER_RC_CHECK(config->eadc.spec_vbus != NULL ? 0 : -ENOTSUP);

	/* Vref */
	NUMAKER_RC_CHECK(numaker_eadc_vref_init(dev));
...
int numaker_utcpd_vbus_set_discharge(const struct device *dev, bool enable)
{
	...
	if (vbus_discharge_spec->port != NULL) {
		rc = gpio_pin_set_dt(vbus_discharge_spec, enable);
		goto cleanup;
	}
	...
	NUMAKER_UTCPD_REG_WRITE_BY_NAME(dev, PWRCTL, pwrctl);
...
static int numaker_utcpd_bist_test_mode_set_enable(const struct device *dev, bool enable)
{
	...
	NUMAKER_UTCPD_REG_WRITE_BY_NAME(dev, CTL, ctl);
...
static int numaker_utcpd_rx_fifo_enqueue(const struct device *dev)
{
	...
	if (rxbcnt < 3) {
		LOG_ERR("Invalid UTCPD_RXBCNT: %d", rxbcnt);
		rc = -EIO;
		goto cleanup;
	}
...
/* Configure EADC sample module with trigger source, channel, etc. */
				     ...
				     uint32_t trgsel)
{
	...
	NUMAKER_RC_CHECK_WITH_MSG((acq_time_unit == ADC_ACQ_TIME_TICKS) ? 0 : -ENOTSUP,
				  "Invalid acquisition time unit for VBUS/VCONN");
...
static int numaker_utcpd_vbus_init(const struct device *dev)
{
	...
	NUMAKER_UTCPD_REG_WRITE_BY_NAME(dev, VBVOL, vbvol);
	...
	if (config->eadc.spec_vbus != NULL) {
		...
		NUMAKER_UTCPD_VBUS_THRESHOLD_WRITE(dev, VBOVTH,
						   NUMAKER_UTCPD_VBUS_THRESHOLD_OVERVOLTAGE);

		/* UTCPD VBUS vSafe5V threshold */
		NUMAKER_UTCPD_VBUS_THRESHOLD_WRITE(dev, VSAFE5V,
						   NUMAKER_UTCPD_VBUS_THRESHOLD_VSAFE5V);

		/* UTCPD VBUS vSafe0V threshold */
		NUMAKER_UTCPD_VBUS_THRESHOLD_WRITE(dev, VSAFE0V,
						   NUMAKER_UTCPD_VBUS_THRESHOLD_VSAFE0V);

		/* UTCPD VBUS stop force discharge threshold */
		NUMAKER_UTCPD_VBUS_THRESHOLD_WRITE(
			dev, SPDGTH, NUMAKER_UTCPD_VBUS_THRESHOLD_STOP_FORCE_DISCHARGE);

		/* UTCPD VBUS sink disconnect threshold */
		NUMAKER_UTCPD_VBUS_THRESHOLD_WRITE(dev, SKVBDCTH,
						   NUMAKER_UTCPD_VBUS_THRESHOLD_SINK_DISCONNECT);
	}
...
static int numaker_utcpd_gpios_init(const struct device *dev)
{
	...
	NUMAKER_RC_CHECK_WITH_MSG((spec->port != NULL ? 0 : -ENODEV),
				  "VBUS detect pin not specified");
	NUMAKER_RC_CHECK_WITH_MSG((gpio_is_ready_dt(spec) ? 0 : -ENODEV),
				  "VBUS detect pin port device not ready");
	NUMAKER_RC_CHECK_WITH_MSG(gpio_pin_configure_dt(spec, GPIO_INPUT),
				  "VBUS detect pin configured to INPUT failed");

	/* Configure VBUS discharge pin to OUTPUT INACTIVE */
	spec = &config->utcpd.gpios.vbus_discharge;
	if (spec->port != NULL) {
		NUMAKER_RC_CHECK_WITH_MSG((gpio_is_ready_dt(spec) ? 0 : -ENODEV),
					  "VBUS discharge pin port device not ready");
		NUMAKER_RC_CHECK_WITH_MSG(
			gpio_pin_configure_dt(spec, GPIO_OUTPUT_INACTIVE),
			"VBUS discharge pin configured to OUTPUT INACTIVE failed");
	}
	...
	if (spec->port != NULL) {
		NUMAKER_RC_CHECK_WITH_MSG((gpio_is_ready_dt(spec) ? 0 : -ENODEV),
					  "VCONN discharge pin port device not ready");
		NUMAKER_RC_CHECK_WITH_MSG(
			gpio_pin_configure_dt(spec, GPIO_OUTPUT_INACTIVE),
			"VCONN discharge pin configured to OUTPUT INACTIVE failed");
	}
...
static int numaker_utcpd_phy_init(const struct device *dev)
{
	...
	NUMAKER_UTCPD_REG_WRITE_BY_NAME(dev, PHYCTL, phyctl);
...
static int numaker_utcpd_interrupts_init(const struct device *dev)
{
	...
	NUMAKER_UTCPD_REG_WRITE_BY_NAME(dev, IE, ie);
	...
	NUMAKER_UTCPD_REG_WRITE_BY_NAME(dev, PWRSTSIE, pwrstsie);
	...
	NUMAKER_UTCPD_REG_WRITE_BY_NAME(dev, FUTSTSIE, futstsie);

	/* VNDIE */
	vndie = UTCPD_VNDIE_VCDGIE_Msk | UTCPD_VNDIE_CRCERRIE_Msk | UTCPD_VNDIE_TXFRSIE_Msk |
		UTCPD_VNDIE_RXFRSIE_Msk;
	NUMAKER_UTCPD_REG_WRITE_BY_NAME(dev, VNDIE, vndie);
...
static int numaker_utcpd_init_startup(const struct device *dev)
{
	...
	/* UTCPD GPIO */
	NUMAKER_RC_CHECK(numaker_utcpd_gpios_init(dev));

	/* UTCPD PHY */
	NUMAKER_RC_CHECK(numaker_utcpd_phy_init(dev));

	/* UTCPD Dead Battery */
	NUMAKER_RC_CHECK(numaker_utcpd_deadbattery_init(dev));

	/* UTCPD pin polarity */
	pinpl = config->utcpd.pinpl.bit;
	NUMAKER_UTCPD_REG_WRITE_BY_NAME(dev, PINPL, pinpl);

	/* VBUS voltage and monitor */
	NUMAKER_RC_CHECK(numaker_utcpd_vbus_init(dev));
	...
	NUMAKER_UTCPD_REG_WRITE_BY_NAME(dev, FUTCTL, futctl);
	...
	NUMAKER_UTCPD_REG_WRITE_BY_NAME(dev, MUXSEL, muxsel);

	/* Interrupts */
	NUMAKER_RC_CHECK(numaker_utcpd_interrupts_init(dev));
...
/* Initialize EADC to be timer-triggered for measuring VBUS/VCONN voltage */
static int numaker_eadc_init_startup(const struct device *dev)
{
	...
	if (spec) {
		NUMAKER_RC_CHECK(numaker_eadc_smplmod_init(dev, spec, config->eadc.trgsel_vbus));
	}
	...
	if (spec) {
		NUMAKER_RC_CHECK(numaker_eadc_smplmod_init(dev, spec, config->eadc.trgsel_vconn));
	}
...
/* Initialize Timer to trigger EADC for measuring VBUS/VCONN voltage */
...
/* TCPC initialize at attach/detach recycle */
static int numaker_tcpc_init_recycle(const struct device *dev)
{
	...
	NUMAKER_RC_CHECK(numaker_utcpd_init_recycle(dev));
...
/* TCPC initialize at device startup */
static int numaker_tcpc_init_startup(const struct device *dev)
{
	...
	NUMAKER_RC_CHECK(pinctrl_apply_state(config->pincfg, PINCTRL_STATE_DEFAULT));

	/* Invoke Clock controller to enable module clock */

	/* Equivalent to CLK_EnableModuleClock() */
	NUMAKER_RC_CHECK(
		clock_control_on(config->clkctrl_dev, (clock_control_subsys_t)&config->pcc_utcpd));
	NUMAKER_RC_CHECK(
		clock_control_on(config->clkctrl_dev, (clock_control_subsys_t)&config->pcc_timer));

	/* Equivalent to CLK_SetModuleClock() */
	NUMAKER_RC_CHECK(clock_control_configure(config->clkctrl_dev,
	...
	NUMAKER_RC_CHECK(reset_line_toggle_dt(&config->reset_utcpd));
	NUMAKER_RC_CHECK(reset_line_toggle_dt(&config->reset_timer));

	/* Initialize UTCPD */
	NUMAKER_RC_CHECK(numaker_utcpd_init_startup(dev));

	if (config->eadc.spec_vbus != NULL || config->eadc.spec_vconn != NULL) {
		/* Initialize EADC */
		NUMAKER_RC_CHECK(numaker_eadc_init_startup(dev));

		/* Initialize Timer */
		NUMAKER_RC_CHECK(numaker_timer_init_startup(dev));
	}

cleanup:

	if (rc == 0) {
		rc = numaker_tcpc_init_recycle(dev);
	}
...
static int numaker_tcpc_set_cc(const struct device *dev, enum tc_cc_pull pull)
{
	 ...
	 */
	NUMAKER_RC_CHECK(numaker_utcpd_deadbattery_query_enable(dev));
	if (rc == 1) {
		NUMAKER_RC_CHECK(numaker_utcpd_deadbattery_set_enable(dev, false));
	}
	...
	NUMAKER_UTCPD_REG_WRITE_BY_NAME(dev, ROLCTL, rolctl);
...
static int numaker_tcpc_vconn_discharge(const struct device *dev, bool enable)
{
	...
	if (data->dpm.vconn_discharge_cb) {
		rc = data->dpm.vconn_discharge_cb(dev, polarity, enable);
		goto cleanup;
	}

	/* Use GPIO VCONN discharge */
	if (vconn_discharge_spec->port != NULL) {
		rc = gpio_pin_set_dt(vconn_discharge_spec, enable);
		goto cleanup;
	}
	...
	NUMAKER_UTCPD_REG_WRITE_BY_NAME(dev, VCDGCTL, vcdgctl);
...
static int numaker_tcpc_set_vconn(const struct device *dev, bool enable)
{
	...
	if (data->dpm.vconn_cb) {
		rc = data->dpm.vconn_cb(dev, polarity, enable);
		goto cleanup;
	}
				  ...
				  enum tc_data_role data_role)
{
	...
	NUMAKER_UTCPD_REG_WRITE_BY_NAME(dev, MSHEAD, mshead);
...
static int numaker_tcpc_set_rx_enable(const struct device *dev, bool enable)
{
	...
	NUMAKER_UTCPD_REG_WRITE_BY_NAME(dev, DTRXEVNT, dtrxevnt);
...
static int numaker_tcpc_transmit_data(const struct device *dev, struct pd_msg *msg)
{
	...
	NUMAKER_UTCPD_REG_WRITE_BY_NAME(dev, TXBCNT, msg->len + 2);

	/* Tx header */
	NUMAKER_UTCPD_REG_WRITE_BY_NAME(dev, TXHEAD, msg->header.raw_value);

	/* Tx data */
	{
		uint32_t data_rmn = msg->len;
		const uint8_t *data_pos = msg->data;
		...
		while (data_rmn >= 4) {
			...
			NUMAKER_UTCPD_REG_WRITE_BY_OFFSET(dev, data_reg_offset, data_value);
	...
	NUMAKER_UTCPD_REG_WRITE_BY_NAME(dev, TXCTL, txctl);
...
int numaker_tcpc_vbus_measure(const struct device *dev, int *vbus_meas)
{
	...
	NUMAKER_RC_CHECK(vbus_meas != NULL ? 0 : -EINVAL);
	*vbus_meas = 0;

	NUMAKER_RC_CHECK(numaker_utcpd_vbus_measure(dev, &mv));
	*vbus_meas = mv;
...
/* NOTE: These indexes must be integer literal, or meet macro expansion error.
