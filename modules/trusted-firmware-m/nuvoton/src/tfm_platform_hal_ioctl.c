...
NVT_TFM_PLAT_IOCTL_SRV_DECL(SYS_ResetModule_Assert)
{
	...
	if (!nvt_check_sys_ns(u32ModuleIndex)) {
...
NVT_TFM_PLAT_IOCTL_SRV_DECL(SYS_GPx_MFPx_Read)
{
	...
	/* Limit access to non-secure range from non-secure domain */
	reg_msk_ns = nvt_get_mfp_mask_ns(reg_addr);
	reg_val &= reg_msk_ns;
...
NVT_TFM_PLAT_IOCTL_SRV_DECL(SYS_GPx_MFPx_Write)
{
	...
	reg_msk_ns = nvt_get_mfp_mask_ns(reg_addr);
	reg_val &= reg_msk_ns;
	reg_val |= *((volatile uint32_t *)reg_addr) & ~reg_msk_ns;
...
NVT_TFM_PLAT_IOCTL_SRV_DECL(SYS_GPx_MFOSx_Read)
{
	...
	/* Limit access to non-secure range from non-secure domain */
	reg_msk_ns = nvt_get_mfos_mask_ns(reg_addr);
	reg_val &= reg_msk_ns;
...
NVT_TFM_PLAT_IOCTL_SRV_DECL(CLK_SetModuleClock)
{
	...
	/* Guard access to secure module from non-secure domain */
	if (!nvt_check_clk_ns(u32ModuleIndex)) {
		return TFM_PLATFORM_ERR_SYSTEM_ERROR;
	}
