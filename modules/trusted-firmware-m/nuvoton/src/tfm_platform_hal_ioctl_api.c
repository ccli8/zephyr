...
#define NVT_TFM_PLAT_IOCTL_CALL(FUNC, INBUF, INSIZE, OUTBUF, OUTSIZE, ARGFMT, ...)                 \
	do {                                                                                       \
		...
		if (rc != TFM_PLATFORM_ERR_SUCCESS) {                                              \
			NVT_FATAL_ERROR(FUNC, rc, ARGFMT, __VA_ARGS__);                            \
		}                                                                                  \
	} while (0)

#if defined(__ZEPHYR__)
#define NVT_FATAL_ERROR(FUNC, RC, ARGFMT, ...)                                                     \
	do {                                                                                       \
		printk(#FUNC ARGFMT " failed (%d)\n", __VA_ARGS__, RC);                            \
		k_oops();                                                                          \
	} while (0)
...
void NVT_TFM_PLAT_IOCTL_NS(SYS_LockReg)(void)
{
	...
	NVT_TFM_PLAT_IOCTL_CALL(SYS_LockReg, NULL, 0, NULL, 0, "(%s)", "");
}

void NVT_TFM_PLAT_IOCTL_NS(SYS_UnlockReg)(void)
{
	/* Invoke Platform IOCTL function */
	NVT_TFM_PLAT_IOCTL_CALL(SYS_UnlockReg, NULL, 0, NULL, 0, "(%s)", "");
}
...
void NVT_TFM_PLAT_IOCTL_NS(SYS_ResetModule_Assert)(uint32_t u32ModuleIndex)
{
	...
	/* Invoke Platform IOCTL function */
	NVT_TFM_PLAT_IOCTL_CALL(SYS_ResetModule_Assert, inbuf, sizeof(inbuf), NULL, 0, "(0x%08x)",
				u32ModuleIndex);
}
...
void NVT_TFM_PLAT_IOCTL_NS(SYS_ResetModule_Deassert)(uint32_t u32ModuleIndex)
{
	...
	/* Invoke Platform IOCTL function */
	NVT_TFM_PLAT_IOCTL_CALL(SYS_ResetModule_Deassert, inbuf, sizeof(inbuf), NULL, 0, "(0x%08x)",
				u32ModuleIndex);
}
...
bool NVT_TFM_PLAT_IOCTL_NS(SYS_ResetModule_IsAsserted)(uint32_t u32ModuleIndex)
{
	...
	NVT_TFM_PLAT_IOCTL_CALL(SYS_ResetModule_IsAsserted, inbuf, sizeof(inbuf), outbuf,
				sizeof(outbuf), "(0x%08x)", u32ModuleIndex);
...
void NVT_TFM_PLAT_IOCTL_NS(SYS_GPx_MFPx_Write)(uint32_t reg_addr, uint32_t reg_val)
{
	...
	NVT_TFM_PLAT_IOCTL_CALL(SYS_GPx_MFPx_Write, inbuf, sizeof(inbuf), NULL, 0,
				"(0x%08x, 0x%08x)", reg_addr, reg_val);
}
...
void NVT_TFM_PLAT_IOCTL_NS(SYS_USBPHY_Write)(uint32_t reg_addr, uint32_t reg_val)
{
	...
	/* Invoke Platform IOCTL function */
	NVT_TFM_PLAT_IOCTL_CALL(SYS_USBPHY_Write, inbuf, sizeof(inbuf), NULL, 0, "(0x%08x, 0x%08x)",
				reg_addr, reg_val);
}
					       ...
					       uint32_t u32ClkDiv)
{
	...
	NVT_TFM_PLAT_IOCTL_CALL(CLK_SetModuleClock, inbuf, sizeof(inbuf), NULL, 0,
				"(0x%08x, 0x%08x, 0x%08x)", u32ModuleIndex, u32ClkSrc, u32ClkDiv);
}
...
void NVT_TFM_PLAT_IOCTL_NS(CLK_EnableModuleClock)(uint32_t u32ModuleIndex)
{
	...
	/* Invoke Platform IOCTL function */
	NVT_TFM_PLAT_IOCTL_CALL(CLK_EnableModuleClock, inbuf, sizeof(inbuf), NULL, 0, "(0x%08x)",
				u32ModuleIndex);
}
...
void NVT_TFM_PLAT_IOCTL_NS(CLK_DisableModuleClock)(uint32_t u32ModuleIndex)
{
	...
	/* Invoke Platform IOCTL function */
	NVT_TFM_PLAT_IOCTL_CALL(CLK_DisableModuleClock, inbuf, sizeof(inbuf), NULL, 0, "(0x%08x)",
				u32ModuleIndex);
}

void NVT_TFM_PLAT_IOCTL_NS(FMC_Open)(void)
{
	/* Invoke Platform IOCTL function */
	NVT_TFM_PLAT_IOCTL_CALL(FMC_Open, NULL, 0, NULL, 0, "%s", "");
}

void NVT_TFM_PLAT_IOCTL_NS(FMC_Close)(void)
{
	/* Invoke Platform IOCTL function */
	NVT_TFM_PLAT_IOCTL_CALL(FMC_Close, NULL, 0, NULL, 0, "(%s)", "");
}

void NVT_TFM_PLAT_IOCTL_NS(FMC_ENABLE_AP_UPDATE)(void)
{
	/* Invoke Platform IOCTL function */
	NVT_TFM_PLAT_IOCTL_CALL(FMC_ENABLE_AP_UPDATE, NULL, 0, NULL, 0, "(%s)", "");
}

void NVT_TFM_PLAT_IOCTL_NS(FMC_DISABLE_AP_UPDATE)(void)
{
	/* Invoke Platform IOCTL function */
	NVT_TFM_PLAT_IOCTL_CALL(FMC_DISABLE_AP_UPDATE, NULL, 0, NULL, 0, "(%s)", "");
}
